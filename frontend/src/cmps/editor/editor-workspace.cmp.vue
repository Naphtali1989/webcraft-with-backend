<template>
    <section :class="emptyWorkspace" class="editor-workspace flex column">
        <Container class="dnd-container" group-name="editor" drag-class="card-ghost" drop-class="card-ghost-drop" :drop-placeholder="dropPlaceholderOptions" :get-child-payload="getChildPayload" @drop="onDrop">
            <Draggable v-for="cmp in cmps" :key="cmp._id">
                <wap-worker :cmp="cmp" @copy="emitCopy" @delete="emitDelete" @clicked="emitUserChoice" @updatedTxt="emitUpdateTxt" @moveSection="emitMoveSection">
                </wap-worker>
            </Draggable>
