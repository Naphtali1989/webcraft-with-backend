  ...
  <section class="user-details-container flex space-between">
    <div class="user-area-container flex column">
      <div class="user-circle">
        <div class="user-img" v-if="loggedInUser.imgUrl">
          <img :src="imgUrl" />
        </div>
      </div>
      <div class="user-info flex column align-center">
        <span class="name"
          ><i class="fas fa-user-circle"></i> {{ loggedInUser.username }}</span
        >
        <span class="email"
          ><i class="far fa-envelope"></i> {{ loggedInUser.email }}</span
        >
        <span class="created-at">Created at {{ formatTime }}</span>
        <label class="user-input input-file"
          ><i class="fas fa-cloud-upload-alt"></i>
          <input class="hide" type="file" @change="setAvatar" />
        </label>
      </div>
    </div>
    ...
    <component v-if="ownerWaps" :is="currTab" :waps="dataToTransfer" />
  </section>
</template>
